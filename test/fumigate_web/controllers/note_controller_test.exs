defmodule FumigateWeb.NoteControllerTest do
  use FumigateWeb.ConnCase

  alias Fumigate.Fragrance

  @create_attrs %{note_name: "some note_name"}
  @update_attrs %{note_name: "some updated note_name"}
  @invalid_attrs %{note_name: nil}

  def fixture(:note) do
    {:ok, note} = Fragrance.create_note(@create_attrs)
    note
  end
  ...
  describe "update note" do
    ...
    test "redirects when data is valid", %{conn: conn, note: note} do
      ...
      assert html_response(conn, 200) =~ "some updated note_name"
    end
